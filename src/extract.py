...
def get_public_holidays(public_holidays_url: str, year: str) -> DataFrame:
    ...
    try:
        ...
        raise SystemExit(f"Error conexión: {e}")
    except requests.RequestException as e:
        raise SystemExit(f"Error conexión: {e}")

    data = response.json()
    dfHolidays = DataFrame(data)
    dfHolidays = dfHolidays.drop(columns=["types", "counties"])
    dfHolidays["date"] = to_datetime(dfHolidays["date"])
